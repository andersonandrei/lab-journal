...
#+TAGS: Book(b) Code(C) Paper(P) Presentation(p) Scheduler(S) Denis(d) Clément(c) Andrei(a) Qarnot(q) WeekReview(w)
...
*** TODO 
**** TODO To talk with Alex to get the current state of the StorageController
...
:END:      
**** WAITING To generate different kinds of workloads (more taks than resources) to test it.
:LOGBOOK:  
...
** thursday, 07-03-2019
*** TODO
**** WAITING To make the qarnotScheduler works with 1week workload :Clément:
:LOGBOOK:  
...
:END:      
**** WAITING To get one day more of simulation data using the qarnot-extractor
:LOGBOOK:  
...
***** Solved.
**** There was a Bug on the updateAndReport() on the QBox level. It was getting wrong the available resources, then inform wrong values to the doDispatch() on the QNode level.
...
*** DONE Mater2 -> Register my presentation in June with an external expert.
:LOGBOOK:  
...
*** WAITING The script to analyze the _temperatures.csv with R.
:LOGBOOK:  
...
** REPORT I finished the first version of the diagrams (real platform and simulations)
* Week 21-03 / 27-03
...
- State "DONE"       from "TODO"       [2019-03-28 qui 13:17]
:END:      
*** TODO To check and fix the JobCompletio TODO in the NodeSched.
...
:END:      
** tuesday, 26-03-2019
*** TODO To work on the scheduler
**** TODO ReSchedule a job killed because it has lower priority than a new one.
...
** REPORT I have continuing waiting for the StorageController implementatio.
** REPORT I am debugging the last version of Clément and testing differente workloads.
** REPORT I have started to work on the re-submition
** REPORT I fininshed the first plots for the temperature analyzes (1. looking for the means of all rads; 2. looking for whole data for some specific QRads)
* Week 28-03 / 03-04
...
*** TODO To Check: Is the availableMobos method working properly?? Because we are killing jobs but it shows available mobos !!!
**** Line 337 - BoxSched: # Some instances were dispatched by cannot be started yet, return them to the QNode
** friday, 29-03-2019
*** TODO To check the train to Paris and send to samantha.sanchez@imag.fr
